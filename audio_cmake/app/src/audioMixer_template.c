...
void AudioMixer_queueSound(wavedata_t *pSound)
{
    ...
    int full = 1; // var to check if array is full or not
    pthread_mutex_lock(&audioMutex); // ensure access is threadsafe
    
    for (int i = 0; i < MAX_SOUND_BITES; i++) {
        if (soundBites[i].pSound == NULL) {
            // found an open spot for update full and insert new sound into spot
			full = 0;
    ...
    pthread_mutex_unlock(&audioMutex); // unlock

	// if full, write error message
    if (full) {
        printf("Error: soundBites is full. Cannot add to it.\n");
    }
...
static void fillPlaybackBuffer(short *buff, int size)
{
    ...
    pthread_mutex_lock(&audioMutex); // ensure synchronization
    ...
    for (int i = 0; i < MAX_SOUND_BITES; i++) {
        if (soundBites[i].pSound != NULL) {
            ...
            // Copy into buffer
            for (int j = 0; j < size && loc < pSound->numSamples; j++) {
                int check = buff[j] + pSound->pData[loc];
                
                if (check > SHRT_MAX) {
                    check = SHRT_MAX;
                } else if (check < SHRT_MIN) {
                    check = SHRT_MIN;
                }
